...
COPY
(SELECT 'timeout 15 /Applications/QGIS.app/Contents/MacOS/QGIS --snapshot ./' || postcode || '.png --extent ' || ROUND(ST_XMin(geom)::numeric, 4) || ',' || ROUND(ST_YMin(geom)::numeric, 4) || ',' || ROUND(ST_XMax(geom)::numeric, 4) || ',' || ROUND(ST_YMax(geom)::numeric, 4) || ' --project ./open_nz_post.qgz --nologo --noversioncheck --hide-browser' FROM postcode_boundaries ORDER BY postcode)
 TO '/app/snapshots/qgis_screenshots.sh';
